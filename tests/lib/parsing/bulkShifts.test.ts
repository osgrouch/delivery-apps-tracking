...
describe("parseBulkShiftsText", () => {
  it("parses a single shift with an explicit date and no colon in the time range", () => {
    ...
    const result = parseBulkShiftsText(text, apps, locations, referenceDate);
  ...
  it("reuses the last location across multiple shifts until a new one appears", () => {
    const text = [
      "6/30",
      "Rochester, NY",
      "Uber Eats",
    ...
    expect(result.issues).toEqual([]);
    expect(result.shifts).toHaveLength(3);
    expect(result.shifts[0]).toMatchObject({ locationId: 1, locationName: "Rochester, NY" });
    expect(result.shifts[1]).toMatchObject({ locationId: 1, locationName: "Rochester, NY" });
    expect(result.shifts[2]).toMatchObject({ locationId: 2, locationName: "Williamsport, PA" });
  });

  it("leaves location null (not an issue) when no location line has appeared yet", () => {
    const text = ["6/30", "Uber Eats", "100", "20", "5", "900-1200"].join("\n");

    const result = parseBulkShiftsText(text, apps, locations, referenceDate);

    expect(result.issues).toEqual([]);
    expect(result.shifts[0]).toMatchObject({ locationId: null, locationName: null });
  });
